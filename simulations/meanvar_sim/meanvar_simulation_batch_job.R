...
tols <- c(1e-3, 1e-5, 1e-7, 1e-10)
deltas <- c(0.1, 1.1)
priors <- c(1e-1, 1e-3, 1e-5)
...
# simulation settings
settings <- data.frame(T = c(rep(100, 2), rep(500, 6), rep(1000, 6)),
                       L = c(2, 5, rep(c(2, 5, 10), 4)),
                       min_space = c(15, 15, rep(15, 3), rep(30, 3), rep(30, 3), rep(50, 3)))
...
for (j in 1:nrow(settings)) {
  ...
  result[nrow(result),-1] <- c(T, L, min_space, NA, NA, NA, time, NA, L_est, n_detected,
  ...
  result[nrow(result),-1] <- c(T, L, min_space, NA, NA, NA,
                               time, NA, L_est, NA, NA, NA,
                               mean_mse, var_mse,
  ...
  for (prior in priors) {
    ...
    #### MICH (J,0,0) Auto ####
    # fit model and time process
    time <- system.time({fit <- mich(cp_data$y, J_auto = TRUE, max_iter = Inf,
                                     tol = tol, reverse = FALSE, restart = TRUE,
                                     omega_j = prior, u_j = prior, v_j = prior)})[3]
    ...
    time <- system.time({fit_rev <- mich(cp_data$y, J_auto = TRUE, max_iter = Inf,
                                         tol = tol, reverse = TRUE, restart = TRUE,
                                         omega_j = prior, u_j = prior, v_j = prior)})[3]
    ...
    # fit model and time process
    time <- system.time({fit <- mich(cp_data$y, J_auto = TRUE, max_iter = Inf,
                                     tol = tol, reverse = FALSE, restart = FALSE,
                                     omega_j = prior, u_j = prior, v_j = prior)})[3]
    ...
    time_rev <- time
    time <- system.time({fit_rev <- mich(cp_data$y, J_auto = TRUE, max_iter = Inf,
                                         tol = tol, reverse = TRUE, restart = FALSE,
                                         omega_j = prior, u_j = prior, v_j = prior)})[3]
    time <- max(time, time_rev)
    if (max(fit$elbo) < max(fit_rev$elbo)) {
      fit <- fit_rev
    ...
    #### MICH (J,0,0) Oracle ####
    # fit model and time process
    time <- system.time({fit <- mich(cp_data$y, J = L, max_iter = Inf,
                                     tol = tol, reverse = FALSE, restart = FALSE,
                                     omega_j = prior, u_j = prior, v_j = prior)})[3]
    ...
    # fit model and time process
    time_rev <- time
    time <- system.time({fit_rev <- mich(cp_data$y, J = L, max_iter = Inf,
                                         tol = tol, reverse = TRUE, restart = FALSE,
                                         omega_j = prior, u_j = prior, v_j = prior)})[3]
...
write.csv(result, paste0("~/MICH/results/result_",jobid,".csv"), row.names = FALSE)
