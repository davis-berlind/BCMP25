...
png("sim_plot_med_slides.png", width = 1425, height = 1425/2)
results %>%
...
png("sim_plot.png", width = 1100, height = 600)
results %>%
...
png("sim_med_plot.png", width = 1100, height = 600)
results %>%
  filter(method %in% methods) %>%
...
png("sim_dist_plot.png", width = 1100, height = 650)
results %>%
  filter(method %in% methods) %>%
  mutate(method = gsub("MICH AutoFast Rev", "MICH-Auto", method)) %>%
  mutate(method = gsub("HSMUCE", "H-SMUCE", method)) %>%
...
png("low_full_sim_dist_plot.png", width = 1100, height = 800)
results %>%
  filter(method %in% methods) %>%
...
png("high_full_sim_dist_plot.png", width = 1100, height = 800)
results %>%
  filter(method %in% methods) %>%
